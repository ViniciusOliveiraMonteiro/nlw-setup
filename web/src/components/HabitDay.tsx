...
export function HabitDay({ defaultCompleted = 0, amount = 0, date }: HabitDayProps){
  ...
  const formatedDayMonth = dayjs(date).format('DD/MM');
  const formatedWeekDay = dayjs(date).format('dddd');
    ...
    <Popover.Root>
        ...
        <Popover.Portal>
          <Popover.Content className="min-w-[320px] p-6 rounded-2xl bg-zinc-900 flex flex-col">
            <span className="font-semibold text-zinc-400">{formatedWeekDay}</span>
            <span className="mt-1 font-extrabold leading-tight text-3xl">{formatedDayMonth}</span>
